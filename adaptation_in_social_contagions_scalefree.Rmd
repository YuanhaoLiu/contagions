...
```{r message=FALSE, warning=FALSE}
#scale free network assocaited diffusion function. Corrections start from core nodes 
BA_network_pre_to_core = function(n=500, #number of agents
                           m=50,
                    ...
                    ScaleFree = T) #T = Scale Free topology, F = fully-connected net
                    ...
                    #rewire_proportion = .10) #proportion of rewired edges in small-world; doesn't affect fully-connected nets
{
  ...
  if(ScaleFree == T) {
    net = barabasi.game((m+n), m=5, directed=F)
  }
...
```{r message=FALSE, warning=FALSE}
#scale free network assocaited diffusion function. Corrections start from preriphery nodes
BA_network_core_to_pre = function(n=500, #number of agents
                           m=50,
                    k=6, #number of preference dimensions
                    d=.75, #decay parameter (lower D = faster decay; vary in range 0-1)
                    iters = 5000000, #number of ticks the model will run for
...
```{r message=FALSE, warning=FALSE}
#small world network
SM_network_random_change = function(n=500, #number of agents
                           m=50,
                           k=6, #number of preference dimensions
                           d=.75, #decay parameter (lower D = faster decay; vary in range 0-1)
                           iters = 5000000, #number of ticks the model will run for
                           measurement_interval = 100000, #how many ticks between population measurements
                           SmallWorld = T) #T = small-world topology, F = fully-connected net
                    #caves = 1, #number of caves in small-world; doesn't affect fully-connected nets
                    #rewire_proportion = .10) #proportion of rewired edges in small-world; doesn't affect fully-connected nets
  
{
  ...
  #Create data frame with one row per agent and one column/variable per preference
  #Each cell will be given a random value between -1 and 1  
  p1 = data.frame(matrix(rnorm((n)*k, mean= 1, var=0.5), nrow = n, ncol = k))
  p2 = data.frame(matrix(rnorm((m)*k, mean= -1, var=0.5), nrow = m, ncol = k))
  p = rbind(p1,p2)
  #2 - Associations
  #Create an empty list
  a = list()
  ...
  #Create network topology
  if(SmallWorld == T) {
    net = sample_smallworld(1, (n+m), 5, 0.1)
  }
  ...
  for(i in 1:iters) {
    ...
    if( (i/measurement_interval) == round(i/measurement_interval) | i == 1) {
      ...
      for(x in 1:(n+m)) {
        for(y in 1:(n+m)) {
          pref_sim_mat[x, y]=cor(as.numeric(p[x,]), as.numeric(p[y,]))
          pref_mean[x, y]= mean((as.numeric(p[x,])+as.numeric(p[y,]))/2)
        }
      }
...
```{r message=FALSE, warning=FALSE}
#scale free network 10% core actors accept a correction 
results1=BA_network_core_to_pre(n=45, m = 5, k=6, d=0.95, iters = 50000, measurement_interval = 1000, ScaleFree = T)
...
```{r message=FALSE, warning=FALSE}
#scale free network 10% peripheral actors accept a correction 
results2=BA_network_pre_to_core(n=45, m = 5, k=6, d=0.95, iters = 50000, measurement_interval = 1000, ScaleFree = T)
ggplot(results2, aes(x=tick))+ 
  (aes(y=pref_mean, colours = "hp")) +
  geom_point()
```
...
results3=SM_network_random_change(n=45, m = 5, k=6, d=0.95, iters = 50000, measurement_interval = 1000, ScaleFree = T)
ggplot(results3, aes(x=tick))+ 
  (aes(y=pref_mean, colours = "hp")) +
  geom_point()
```

```{r}
results1=BA_network_core_to_pre(n=900, m = 100, k=6, d=0.95, iters = 5000000, measurement_interval = 10000, ScaleFree = T)
results2=BA_network_pre_to_core(n=900, m = 100, k=6, d=0.95, iters = 5000000, measurement_interval = 10000, ScaleFree = T)

write.csv(results1, "c2p_900_100.csv")
write.csv(results2, "p2c_900_100.csv")
```
